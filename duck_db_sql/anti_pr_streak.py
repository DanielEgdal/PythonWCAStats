...
db = duckdb_reader(['results','competitions','round_types','persons'])
...
where 
--r.person_country_id = 'Denmark' and 
average > 0)
...
, comps_ordered as (
select r.person_id, c.id as comp, make_date(c.year, c.end_month, c.end_day) as end_date, p.country_id
...
, streak_group as (
select *,
SUM(CASE WHEN pr_at_comp_flag = true THEN 1 ELSE 0 END) OVER (PARTITION BY person_id ORDER BY person_id, comp_number) streak_group
from pr_overview)

select person_id
, min(end_date) first_comp_without_pr_date, min_by(comp,end_date) first_comp_without_pr
, max(end_date) last_comp_without_pr_date, max_by(comp,end_date) last_comp_without_pr
, count(*) as comps_without_pr
from streak_group
where pr_at_comp_flag = false
group by streak_group, person_id
-- having comps_without_pr >= 5
order by comps_without_pr desc
