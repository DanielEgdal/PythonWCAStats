...
select person_id, chok_count, total_solves, chok_count/total_solves as ratio, competition_id, comp_date
from grouped_counts g
join comp_dates c
on c.id = g.competition_id
where country_id = 'Denmark'
--qualify row_number() over(partition by person_id order by chok_count desc) = 1
order by chok_count desc
